...
function buildDescription(donut) {
  let description = `${donut.base.name} donut`;
  ...
  results.forEach((result) => {
    if (donuts[result.donut_id] === undefined) {
      ...
      if (result.topping_name !== null) {
        donuts[result.donut_id].toppings = [{
          name: result.topping_name,
          price: result.topping_price,
          id: result.topping_id,
        }];
      }
    } else {
      donuts[result.donut_id].toppings.push({
        name: result.topping_name,
        price: result.topping_price,
        id: result.topping_id,
      });
      donuts[result.donut_id].price += result.topping_price;
    }
  });
